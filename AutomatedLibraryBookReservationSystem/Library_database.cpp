...
string Library_Database::getCurrentDateAsString() {
...
string Library_Database::calculateReturnDate(const std::string& currentDate, const std::string& type) {
    std::tm date = {};
...
void Library_Database::disPlayBooks() 
{
    cout << "---------------------------------------------------------------------\n";
    ...
    for (int i = 0; i < index; i++)
    {
        cout << std::left << setw(15) << books[i].get_title() << setw(15) << books[i].get_author() << setw(15) << books[i].get_isbn() << setw(15) << books[i].reservationStatus() << setw(15) << books[i].get_due_date() << endl;
    }
    cout << "---------------------------------------------------------------------\n";
    cout << endl;
}
        ...
        if (books[i].get_isbn() == query || books[i].get_title() == query)
        {
            ...
            while (true) {
                ...
                while (!(cin >> choice) || !(choice >= 1 && choice <= 6)) {
                    ...
                    cout << "\nERROR! INVALID INPUT ENTERED, PLEASE ENTER AN INTEGER VALUE BETWEEN 1 AND 6\n";
...
bool Library_Database::searchuser() {
    ...
    for (int i = 0; i < index; i++)
    {
        ...
        {
            cout << "\n==========================================================\n";
            cout << "| BOOK EXISTS IN THE DATABASE WITH THE STATUS: " << books[i].reservationStatus() << " |\n";
            cout << "==========================================================\n\n";
            exists = true;
            break;
        }
...
bool Library_Database::search() {
    ...
    for (int i = 0; i < index; i++)
    {
        if (books[i].get_isbn() == isbn || books[i].get_title() == isbn)
        {
            cout << "\nBOOK EXISTS IN THE DATABASE WITH THE STATUS: " << books[i].reservationStatus() << endl;
            exists = true;
        }
    }
    if (!exists)
...
Book* Library_Database::borrow_book(string type)
{
    string isbn;
    ...
    for (int i = 0; i < index; i++)
    {
        if (books[i].get_isbn() == isbn || books[i].get_title() == isbn)
        {
            if (!books[i].checkBorrowed()) {
                books[i].set_borrowed(true);
                ptr = &books[i];
                string currentDate = getCurrentDateAsString();
                string returnDate = calculateReturnDate(currentDate,type);
    ...
    if(!already){
        ...
        if (option == 'Y' || option == 'y')
        {
            books[index2].setReservation(true);
            cout << "\n==============================\n";
            cout << "| BOOK RESERVED SUCCESSFULLY |\n";
            cout << "\n*** Will Be Issue On : " << books[index2].get_due_date() << " ***\n";
            cout << "\n==============================\n\n";
            system("clear");
        }
    }
